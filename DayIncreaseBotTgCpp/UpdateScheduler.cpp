...
void UpdateScheduler::sendDailyMessage(int64_t chatId) {
    ...
    const std::time_t today_time = std::chrono::system_clock::to_time_t(today);
    std::tm today_tm = {};

    localtime_r(&today_time, &today_tm);

    const std::time_t raw_time = std::mktime(&today_tm);
...
SolsticeStatus UpdateScheduler::getSolsticeStatus(const std::chrono::system_clock::time_point &currentDate) {
    const int currentYear = SolsticeData::getYearFromDate(currentDate);
    ...
    const bool isSolsticeDay = (currentDate == winterSolstice || currentDate == summerSolstice);

    const std::string solsticeType = (isSolsticeDay) ? (currentDate == winterSolstice ? "winter" : "summer") : "";

    const bool isDaylightIncreasing = (currentDate > winterSolstice && currentDate < summerSolstice);

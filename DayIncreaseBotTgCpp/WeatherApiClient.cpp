...
std::string WeatherApiClient::getTzId(const double latitude, const double longitude) const {
    const std::string tzUrl = m_tzApiUrl + "?key=" + m_apiKey + "&format=json&by=position&lat=" +
                              std::to_string(latitude) + "&lng=" + std::to_string(longitude);
    try {
...
std::string WeatherApiClient::getWeatherData(const double latitude, const double longitude, const std::string &date,
